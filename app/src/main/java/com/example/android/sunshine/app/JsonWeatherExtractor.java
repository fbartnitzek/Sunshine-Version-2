...
//    }
    ...
    private static Long getUtcTime(String timeStringUtc) throws ParseException{
        // convert utcString to localeLong
        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");//2015-06-21 21:00:00
        return formatter.parse(timeStringUtc).getTime();
//        return date.getTime();
    }


    private static String getReadableDetailString(long time) throws ParseException {
        // Because the API returns a unix timestamp (measured in seconds),
        // it must be converted to milliseconds in order to be converted to valid date.
    ...
    public static List<String[]> getDetailedWeatherDataFromJson(String forecastJsonStr, String unitType) throws JSONException {
        ...
        //next problem: before midnight there should be just 1 detail for today...
        ...
        // we start at the day returned by local time. Otherwise this is a mess.
        int julianStartDay = Time.getJulianDay(System.currentTimeMillis(), dayTime.gmtoff);
        Log.v(LOG_TAG, "julianStartDay: " + julianStartDay);
        ...
        int i = -1;
        String oldDate = null;
        int previousDay = julianStartDay;
        ...
        do{
            ...
            int currentDay;
            try {
                long utcTime = getUtcTime(startTimeUtc);
                currentDay = Time.getJulianDay(utcTime, gmtOffset);
                if (previousDay == currentDay){
                    Log.v(LOG_TAG, "same day (prev: " + previousDay + ", currentDay: " + currentDay);
                } else {
                    Log.v(LOG_TAG, "different day (prev: " + previousDay + ", currentDay: " + currentDay);
                }


                timeEntry = getReadableDetailString(utcTime + gmtOffset);
