...
//        SunshineSyncAdapter.syncImmediately(this);
    ...
    public Intent getParentActivityIntent() {
        // if main activity is already running - use it instead of create new one
        return super.getParentActivityIntent().addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
    }

    @Override
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        if (getString(R.string.pref_location_label).equals(key)) {
            // location changed => clear locationStatus
            Utility.resetLocationStatus(this);
            SunshineSyncAdapter.syncImmediately(this);
        } else if (getString(R.string.pref_enable_notification_key).equals(key)) {
            // units changed => update lists accordingly
            getContentResolver().notifyChange(WeatherContract.WeatherEntry.CONTENT_URI, null);
        } else if (getString(R.string.pref_location_status_key).equals(key)) {
            // location status changed => update summary accordingly
