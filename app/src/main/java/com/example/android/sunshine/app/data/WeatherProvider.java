...
public class WeatherProvider extends ContentProvider {
    ...
    public int update(
        ...
        switch (match) {
            case WEATHER: {
                normalizeDate(values);
                impactedRows = db.update(WeatherContract.WeatherEntry.TABLE_NAME, values,
                        selection, selectionArgs);
                break;
            }

            case LOCATION: {
                impactedRows = db.update(WeatherContract.LocationEntry.TABLE_NAME, values,
                        selection, selectionArgs);
                break;
            }
        ...
        switch (match) {
            case WEATHER:
                ...
                try {
                    ...
                    db.endTransaction();    //not yet commited
                }
                getContext().getContentResolver().notifyChange(uri, null);
                return returnCount;
            default:
