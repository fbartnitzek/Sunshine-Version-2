...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
    public final String LOG_TAG = SunshineSyncAdapter.class.getName();
...
// more or less the same like FetchWeatherTask ...
...
//            return null;
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            if (forecastJson.has(OWM_MESSAGE_CODE)) {
                switch (forecastJson.getInt(OWM_MESSAGE_CODE)) {
                    case HttpURLConnection.HTTP_OK:
                        Log.v(LOG_TAG, "location found");
                        break;
                    case HttpURLConnection.HTTP_NOT_FOUND:
                        setLocationStatus(getContext(), LOCATION_STATUS_INVALID);
                        Log.v(LOG_TAG, "location not found - invalid");
                        return;
                    default:
                        setLocationStatus(getContext(), LOCATION_STATUS_SERVER_DOWN);
                        Log.v(LOG_TAG, "location default - server down");
                        return;
                }
            ...
            Log.e(LOG_TAG, e.getMessage(), e);
            e.printStackTrace();
            // TODO: test changes via debugging!
            setLocationStatus(getContext(), LOCATION_STATUS_SERVER_INVALID);
        }
        return;
    }
    ...
    private void updateWidgets() {
        ...
        // only Sunshine app can receive broadcast
        Intent dataUpdatedIntent = new Intent(ACTION_DATA_UPDATED).setPackage(ctx.getPackageName());
        ...
        if (displayNotifications){
            ...
            if (System.currentTimeMillis() - lastSync >= DAY_IN_MILLIS) {
                ...
                if (cursor.moveToFirst()) {
                    ...
                    PendingIntent resultPI = stackBuilder.getPendingIntent(0,
    ...
    private long addLocation(String locationSetting, String cityName, double lat, double lon) {
        ...
        if (locationCursor.moveToFirst()){
            int idx = locationCursor.getColumnIndex(WeatherContract.LocationEntry._ID);
            locationId = locationCursor.getLong(idx);
        } else {
    ...
    static private void setLocationStatus(Context c, @LocationStatus int locationStatus) {
        ...
        spe.commit();   // on background thread commit, on foreground thread apply
    }
}
